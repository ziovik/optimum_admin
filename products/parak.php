                                ...
                                $customer_id = $_SESSION['customer_id'];
                                  ...
                                  if (isset($_GET['par'])) {
                                      ...
                                      $get_par_pro =
                                         ...
                                         where c.id ='$customer_id' and sb.id= '$par_id'";
                              
                                      $run_par_pro = mysqli_query($con, $get_par_pro);
                              
                                      $count_par = mysqli_num_rows($run_par_pro);
                                      if ($count_par == 0) {
                                          echo "<h2 style='text-align:center;'>'Нет продукта'</h2>";
                                      }else{
                              
                                          while($row_par_pro=mysqli_fetch_array($run_par_pro)){
                                              $pro_id = $row_par_pro['product_id'];
                                              $pro_name = $row_par_pro['product_name'];
                                              $pro_manu = $row_par_pro['product_manufacturer'];
                                              $pro_price = $row_par_pro['product_price'];
                                              $pro_dist = $row_par_pro['company_name'];
                                              $pro_min_order = $row_par_pro['product_min_order'];
                                              $pro_expires = $row_par_pro['expires'];
                                              $pro_desc = $row_par_pro['discription'];
                                              ?>
                     <tbody>
                        <tr>
                           <th data-priority="1"
