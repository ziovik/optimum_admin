                                ...
                                $customer_id = $_SESSION['customer_id'];
                                  ...
                                  if (isset($_GET['depil'])) {
                                      ...
                                      $get_depil_pro =
                                          ...
                                          where c.id ='$customer_id' and sb.id= '$depil_id'";
                              
                                      $run_depil_pro = mysqli_query($con, $get_depil_pro);
                              
                                      $count_depil = mysqli_num_rows($run_depil_pro);
                                      if ($count_depil == 0) {
                                          echo "<h2 style='text-align:center;'>'Нет продукта'</h2>";
                                      }else{
                              
                                          while($row_depil_pro=mysqli_fetch_array($run_depil_pro)){
                                              $pro_id = $row_depil_pro['product_id'];
                                              $pro_name = $row_depil_pro['product_name'];
                                              $pro_manu = $row_depil_pro['product_manufacturer'];
                                              $pro_price = $row_depil_pro['product_price'];
                                              $pro_dist = $row_depil_pro['company_name'];
                                              $pro_min_order = $row_depil_pro['product_min_order'];
                                              $pro_expires = $row_depil_pro['expires'];
                                              $pro_desc = $row_depil_pro['discription'];
                                              ?>
                        <tbody>
                        <tr>
                           <th data-priority="1"
