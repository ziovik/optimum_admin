                                 ...
                                 $customer_id = $_SESSION['customer_id'];
                                  ...
                                  if (isset($_GET['eye'])) {
                                      ...
                                      $get_eye_pro =
                                         ...
                                         where c.id ='$customer_id' and sb.id= '$eye_id'";
                              
                                      $run_eye_pro = mysqli_query($con, $get_eye_pro);
                              
                                      $count_eye = mysqli_num_rows($run_eye_pro);
                                      if ($count_eye == 0) {
                                          echo "<h2 style='text-align:center;'>'Нет продукта'</h2>";
                                      }else{
                              
                                          while($row_eye_pro=mysqli_fetch_array($run_eye_pro)){
                                              $pro_id = $row_eye_pro['product_id'];
                                              $pro_name = $row_eye_pro['product_name'];
                                              $pro_manu = $row_eye_pro['product_manufacturer'];
                                              $pro_price = $row_eye_pro['product_price'];
                                              $pro_dist = $row_eye_pro['company_name'];
                                              $pro_min_order = $row_eye_pro['product_min_order'];
                                              $pro_expires = $row_eye_pro['expires'];
                                              $pro_desc = $row_eye_pro['discription'];
                                              ?>
                           <tr>
