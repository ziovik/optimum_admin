                        ...
                        <tr>
                           ...
                           <th colspan="1" data-priority="8">Примечание</th>
                              ...
                              <?php
                                 ...
                                 if (isset($_GET['allmat'])) {
                                                             $allmat_id = $_GET['allmat'];
                                                             $get_allmat_pro =
                                                                 "select 
                                                                   p.id as product_id, 
                                                              ...
                                                              from
                                                                 ...
                                                                 where c.id ='$customer_id' and ct.id= '$allmat_id'";
                                 
                                                             $run_allmat_pro = mysqli_query($con, $get_allmat_pro);
                                 
                                                             
                                    $count_allmat = mysqli_num_rows($run_allmat_pro);
                                 
                                    if ($count_allmat == 0) {
                                        echo "<h2 style='text-align:center;'>Нет продукта</h2>";
                                                             } else {
                                        while($row_allmat_pro=mysqli_fetch_array($run_allmat_pro)) {
                                            $pro_id = $row_allmat_pro['product_id'];
                                            $pro_name = $row_allmat_pro['product_name'];
                                            $pro_manu = $row_allmat_pro['product_manufacturer'];
                                            $pro_price = $row_allmat_pro['product_price'];
                                            $pro_dist = $row_allmat_pro['company_name'];
                                                                     $pro_min_order = $row_allmat_pro['product_min_order'];
                                                                     $pro_expires = $row_allmat_pro['expires'];
                                                                     $pro_desc = $row_allmat_pro['discription'];
