                                 ...
                                 $customer_id = $_SESSION['customer_id'];
                                  ...
                                  if (isset($_GET['mat'])) {
                                      ...
                                      $get_mat_pro =
                                         ...
                                         where c.id ='$customer_id' and sb.id= '$mat_id'";
                              
                                      $run_mat_pro = mysqli_query($con, $get_mat_pro);
                              
                                      $count_mat = mysqli_num_rows($run_mat_pro);
                                      if ($count_mat == 0) {
                                          echo "<h2 style='text-align:center;'>'Нет продукта'</h2>";
                                      }else{
                              
                                          while($row_mat_pro=mysqli_fetch_array($run_mat_pro)){
                                              $pro_id = $row_mat_pro['product_id'];
                                              $pro_name = $row_mat_pro['product_name'];
                                              $pro_manu = $row_mat_pro['product_manufacturer'];
                                              $pro_price = $row_mat_pro['product_price'];
                                              $pro_dist = $row_mat_pro['company_name'];
                                              $pro_min_order = $row_mat_pro['product_min_order'];
                                              $pro_expires = $row_mat_pro['expires'];
                                              $pro_desc = $row_mat_pro['discription'];
                                              ?>
                           <tbody>
                        <tr>
                           <th data-priority="1"
