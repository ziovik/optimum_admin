                                 ...
                                 $customer_id = $_SESSION['customer_id'];
                                  ...
                                  if (isset($_GET['nail'])) {
                                      ...
                                      $get_nail_pro =
                                         ...
                                         where c.id ='$customer_id' and sb.id= '$nail_id'";
                              
                                      $run_nail_pro = mysqli_query($con, $get_nail_pro);
                              
                                      $count_nail = mysqli_num_rows($run_nail_pro);
                                      if ($count_nail == 0) {
                                          echo "<h2 style='text-align:center;'>'Нет продукта'</h2>";
                                      }else{
                              
                                          while($row_nail_pro=mysqli_fetch_array($run_nail_pro)){
                                              $pro_id = $row_nail_pro['product_id'];
                                              $pro_name = $row_nail_pro['product_name'];
                                              $pro_manu = $row_nail_pro['product_manufacturer'];
                                              $pro_price = $row_nail_pro['product_price'];
                                              $pro_dist = $row_nail_pro['company_name'];
                                              $pro_min_order = $row_nail_pro['product_min_order'];
                                              $pro_expires = $row_nail_pro['expires'];
                                              $pro_desc = $row_nail_pro['discription'];
                                              ?>
                           <tbody>
                        <tr>
                           <th data-priority="1"
